...
class SnakeGameAI:
    ...
    def reset(self):
        ...
        self.snake = [self.head,
                      ...
                      #Point(self.head.x-(5*BLOCK_SIZE), self.head.y-(1*BLOCK_SIZE)),
    ...
    def play_step(self, action):
        ...
        game_over = False
        if self.is_collision1():
            game_over = True
            reward = -30# - distance
            return reward, game_over, self.score
        if self.is_collision2() or self.frame_iteration > 100*len(self.snake):
            game_over = True
            reward = -20
            return reward, game_over, self.score
    ...
    def is_collision1(self, pt=None):
        if pt is None:
            pt = self.head
        # hits boundary
    ...
    def is_collision(self, pt = None):
        if pt == None:
            pt = self.head
        if self.is_collision1(pt) or self.is_collision2(pt):
            return True
        return False
